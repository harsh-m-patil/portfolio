    ...
    <Card className="py-0 gap-0">
      <CardHeader className="px-4 py-2 border-b">
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="font-mono">{toolCall.function.name}</Badge>
          <span className="text-muted-foreground text-xs font-mono truncate">{toolCall.id}</span>
        </div>
      </CardHeader>
        ...
        <ScrollArea className="max-h-48">
          <ScrollViewport>
            <pre className="text-sm text-muted-foreground whitespace-pre-wrap break-words font-mono">
              {parsed ? JSON.stringify(parsed, null, 2) : toolCall.function.arguments}
            </pre>
          </ScrollViewport>
        </ScrollArea>
      </CardContent>
      ...
      <Card className="py-0 gap-0">
        <CardHeader className="px-4 py-2 border-b">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground text-xs font-mono truncate">{message.tool_call_id}</span>
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <Card className="py-0 gap-0">
          ...
          <CollapsibleContent>
            <Separator />
            <CardContent className="px-4 py-3">
              <pre className="whitespace-pre-wrap break-words text-sm text-muted-foreground italic">{message.content}</pre>
            </CardContent>
          </CollapsibleContent>
        </Card>
      </Collapsible>
