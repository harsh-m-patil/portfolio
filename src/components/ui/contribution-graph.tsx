...
export const ContributionGraphLegend = ({
  ...
  const { labels, maxLevel, blockSize, blockRadius, themeClass } =
    useContributionGraph();
  ...
  return (
    ...
    >
      ...
      {levels.map(({ id, level }) =>
        children ? (
          <Fragment key={id}>{children({ level })}</Fragment>
        ) : (
          <svg height={blockSize} key={id} width={blockSize}>
